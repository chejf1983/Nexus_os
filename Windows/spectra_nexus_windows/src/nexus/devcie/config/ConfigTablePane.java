...
public class ConfigTablePane extends javax.swing.JPanel {
    ...
    public ConfigTablePane(ISpDevice dev) {
        initComponents();
        this.dev = dev;
        this.InitTable(dev.GetExternConfig());
    }
...
//                    return new DefaultCellRenderer(combox);
            ...
            public TableCellEditor getCellEditor(int row, int column) {
                if (column == 1 && list[row].inputtype == SSConfigItem.ItemType.S) {
                    //设置默认设备名称下拉列表
                ...
                } else if (column == 1 && list[row].inputtype == SSConfigItem.ItemType.B) {
                    //设置默认设备名称下拉列表
                    JCheckBox combox = new JCheckBox();
                    boolean value = Boolean.valueOf(list[row].value);
                    combox.setSelected(value);
...
//        column.setWidth(DataTableModel.column_len[i]);
    ...
    private void Button_setActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Button_setActionPerformed
        ...
        try {
            this.dev.SetExcternConfig(this.tablemodel.GetValues());
        } catch (Exception ex) {
            Logger.getLogger(ConfigTablePane.class.getName()).log(Level.SEVERE, null, ex);
        }
